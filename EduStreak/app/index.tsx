    ...
    const onToggle = () =>
    {
      ...
      useLayoutEffect(() => {
          ...
          useEffect(() => {
            ...
            const dateList = Array.from({ length: range }, (_, i) => getDayLabel(i - half));
            setDates(dateList);
          }, [range]);
        ...
        const renderItem = ({ item }) => {
          ...
          return (
            <TouchableOpacity onPress={() => setSelectedId(item.id)} style={styles.itemWrapper}>
              <View style={[styles.item, isSelected && styles.selectedItem]}>
                <Text style={[styles.day, isSelected && styles.selectedText]}>{item.day}</Text>
                <Text style={[styles.date, isSelected && styles.selectedText]}>{item.date}</Text>
              </View>
            </TouchableOpacity>
          );
        };
